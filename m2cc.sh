...
get_custom_provider_config() {
    echo -e "\n${GREEN}${BOLD}🔧 配置自定义模型提供商${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo
    echo -e "${YELLOW}📝 说明：${NC}"
    echo "   自定义模型提供商允许您配置任意兼容 Anthropic API 的服务。"
    echo "   您需要提供：提供商名称、显示名称、Base URL 和 API Key。"
    echo

    # 获取提供商标识（用于内部识别）
    read -p "请输入提供商标识（仅英文、数字、横杠，如 custom-claude）： " CUSTOM_PROVIDER_ID
    
    if [ -z "$CUSTOM_PROVIDER_ID" ]; then
        log_error "提供商标识不能为空"
        return 1
    fi
    
    # 验证标识格式
    if ! [[ "$CUSTOM_PROVIDER_ID" =~ ^[a-z0-9-]+$ ]]; then
        log_error "提供商标识格式错误，仅支持小写字母、数字和横杠"
        return 1
    fi

    # 获取显示名称
    read -p "请输入显示名称（如 自定义 Claude）： " CUSTOM_PROVIDER_NAME
    
    if [ -z "$CUSTOM_PROVIDER_NAME" ]; then
        CUSTOM_PROVIDER_NAME="$CUSTOM_PROVIDER_ID"
    fi

    # 获取 Base URL
    read -p "请输入 API Base URL： " CUSTOM_BASE_URL
    ...
    # 获取 API Key
    read -p "请输入 API Key/Token： " CUSTOM_API_KEY
    ...
    # 获取超时时间（可选）
    read -p "请输入超时时间（毫秒，默认 600000）： " CUSTOM_TIMEOUT
    CUSTOM_TIMEOUT=${CUSTOM_TIMEOUT:-600000}

    # 获取默认模型名称
    read -p "请输入默认模型名称（默认 claude-3-5-sonnet-20241022）： " CUSTOM_MODEL
    CUSTOM_MODEL=${CUSTOM_MODEL:-claude-3-5-sonnet-20241022}

    # 确认配置
    echo
    echo -e "${CYAN}${BOLD}请确认以下配置：${NC}"
    echo -e "  ${YELLOW}提供商标识：${NC} $CUSTOM_PROVIDER_ID"
    echo -e "  ${YELLOW}显示名称：${NC} $CUSTOM_PROVIDER_NAME"
    echo -e "  ${YELLOW}Base URL：${NC} $CUSTOM_BASE_URL"
    echo -e "  ${YELLOW}API Key：${NC} ${CUSTOM_API_KEY:0:10}****"
    echo -e "  ${YELLOW}超时时间：${NC} $CUSTOM_TIMEOUT ms"
    echo -e "  ${YELLOW}默认模型：${NC} $CUSTOM_MODEL"
    echo

    if ! confirm_action "确认添加此自定义提供商？" "y"; then
        log_warning "已取消配置"
        return 1
    fi
...
configure_custom_provider() {
    ...
    if get_custom_provider_config; then
        configure_provider "custom-$CUSTOM_PROVIDER_ID" "$CUSTOM_API_KEY" \
            "$CUSTOM_PROVIDER_NAME" "$CUSTOM_BASE_URL" "$CUSTOM_TIMEOUT" "$CUSTOM_MODEL"
        return $?
    else
        return 1
    fi
...
# 从环境变量配置自定义提供商（用于脚本自动化）
configure_custom_provider_from_env() {
    local provider_id="$1"
    local provider_name="$2"
    local base_url="$3"
    local api_key="$4"
    local timeout="${5:-600000}"
    local model="${6:-claude-3-5-sonnet-20241022}"

    if [ -z "$provider_id" ] || [ -z "$base_url" ] || [ -z "$api_key" ]; then
        log_error "缺少必需参数：provider_id, base_url, api_key"
        return 1
    fi

    CUSTOM_PROVIDER_ID="$provider_id"
    CUSTOM_PROVIDER_NAME="${provider_name:-$provider_id}"
...
configure_provider() {
    ...
    local api_key=$2
    local custom_display_name="${3:-}"
    local custom_base_url="${4:-}"
    local custom_timeout="${5:-}"
    local custom_model="${6:-}"
    local providers_file="$HOME/.claude/providers.json"
    ...
    if [[ "$provider_name" == custom-* ]]; then
        # 自定义提供商配置
        provider_display="${custom_display_name:-Custom Provider}"
        api_key_url=""
        base_url="$custom_base_url"
        timeout="${custom_timeout:-600000}"
        
        if command -v jq >/dev/null 2>&1; then
            models_json=$(jq -n \
                --arg model "${custom_model:-claude-3-5-sonnet-20241022}" \
                '{
                    "default": $model,
                    "small_fast": $model,
            ...
            models_json=$(cat <<EOF
{
    "default": "${custom_model:-claude-3-5-sonnet-20241022}",
    "small_fast": "${custom_model:-claude-3-5-sonnet-20241022}",
    "DEFAULT_SONNET_MODEL": "${custom_model:-claude-3-5-sonnet-20241022}",
    "DEFAULT_OPUS_MODEL": "${custom_model:-claude-3-5-sonnet-20241022}",
    "DEFAULT_HAIKU_MODEL": "${custom_model:-claude-3-5-sonnet-20241022}"
}
EOF
)
        fi
...
configure_claude_code() {
    ...
    # 自定义模型提供商
    if confirm_action "是否配置自定义模型提供商？" "n"; then
...
选项：
    ...
    -c, --configure         配置或重新配置 AI 模型提供商
    --status                显示当前配置状态
    --add-custom            添加自定义模型提供商（交互式）
    --add-custom-env        从环境变量添加自定义模型提供商
                           需要以下环境变量：
                           PROVIDER_ID, PROVIDER_NAME, BASE_URL, API_KEY
                           可选: TIMEOUT, MODEL
...
支持的预配置提供商：
    ...
    - deepseek       DeepSeek
    - glm            GLM-4.6
    - glm-flash      GLM-4.5-Flash (免费)
    - custom-*       自定义提供商

配置文件位置：
    - 提供商配置：~/.claude/providers.json
    - 活跃配置：~/.claude/settings.json

更多信息：
    GitHub: https://github.com/anthropics
    文档: https://docs.anthropic.com/claude-cli
EOF
}
    ...
    case "$arg1" in
        ...
        --add-custom-env)
            ...
            if [ -z "$PROVIDER_ID" ] || [ -z "$BASE_URL" ] || [ -z "$API_KEY" ]; then
                ...
                echo "  $0 --add-custom-env"
                exit 1
            fi
            
            init_provider_config
            configure_custom_provider_from_env "$PROVIDER_ID" "$PROVIDER_NAME" "$BASE_URL" "$API_KEY" "$TIMEOUT" "$MODEL"
            
            # 配置成功后询问是否立即切换
            if [ $? -eq 0 ]; then
                echo
                if confirm_action "是否立即切换到新添加的提供商？" "y"; then
                    switch_provider "custom-$PROVIDER_ID"
                fi
            fi
            exit $?
            ;;
